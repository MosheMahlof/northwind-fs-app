...
-- Stored procedure: Get a single product by ID
...
-- Stored procedure: Create a product
...
-- Stored procedure: Update a product
...
-- Stored procedure: Delete a product
CREATE OR ALTER PROCEDURE sp_DeleteProduct
    @ProductId INT
AS
BEGIN
    -- Check if product exists in any order detail
    IF EXISTS (SELECT 1 FROM OrderDetails WHERE ProductID = @ProductId)
    BEGIN
        RAISERROR('Cannot delete product as it is referenced in existing orders.', 16, 1);
        RETURN;
    END
...
-- Stored procedure: Get all suppliers
...
-- Stored procedure: Get all customers by order count
CREATE OR ALTER PROCEDURE sp_GetTopCustomersByOrderCount
AS
BEGIN
    SELECT
